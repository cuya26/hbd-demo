              ...
              <div class="table-container">
                <table>
                  <tbody>
                    ...
                    <tr v-else v-for="(row, index) in combinedResults" :key="index"
                      :class="{ 'highlighted': selectedRow === index }"
                      @click="selectRow(index); attachCriteria(row)">
                    <td v-html="row.context"></td>
                    </tr>
                    <tr v-if="loadingPatientSearch || loadingCriteriaCheck" class="loading-container">
                      <td colspan="1">
...
.loading-container {
  ...
  background: rgba(255, 255, 255, 0.8); /* Semi-transparent background to overlay the table */
...
export default defineComponent({
    ...
    attachDocument () {
      if (this.inputLetter != null && this.inputLetter != '') 
        ...
        this.chatHistory.push({ content: 'Rispondi alle domande relative al seguente Testo Clinico: ```' + this.attachedDocument + '```' , role: "user", visible: false })
        this.loadingChatResponse = true
        fetch(llamaHost + '/v1/chat/completions', {
    ...
    attachCriteria(row) {
      ...
      if (text != null && text.trim() !== '') {
        ...
        this.chatHistory.push({ content: 'Rispondi alle domande relative al seguente Testo Clinico: ```' + txt + '```' , role: "user" })
      }
