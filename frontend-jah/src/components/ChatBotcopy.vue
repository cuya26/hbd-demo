  ...
  <q-card>
    ...
    <q-card-section class="" style="height: 90%">
      ...
      <div v-if="!loadingChatBot" class="column justify-center items-center no-wrap col-12" style="height: 100%">
        ...
        <div style="height: 100%; width: 100%; border-radius: 4px; border: 1.5px solid #bdc3c7;" class="overflow-auto q-pa-md" ref="chatWindow">
          <div class="q-px-sm row justify-center" style="height: 100%">
            <div class="col-12">
              <div v-for="chatLine in chatHistory" :key="chatLine" :class="'row justify-' + chatConfig['chatLinePosition'][chatLine.role] + ' q-py-sm'">
                <div :class="'bg-' + chatConfig['chatLineColor'][chatLine.role] + ' q-pa-sm'" style="border-radius: 12px; width: fit-content; max-width: 60%; white-space: pre-line;">
                  {{ chatLine.content }}
                </div>
              </div>
            </div>
        ...
        <div class="row justify-center no-wrap" style="width: 100%">
          ...
          <q-btn icon="cleaning_services" @click="loadingChatResponse ? true : resetChatHistory()" rounded color="warning" dense />
          <div class="q-px-sm"></div>
          <q-btn icon="attach_file" @click="loadingChatResponse ? true : attachDocument()" rounded color="secondary" dense />
          <div class="q-px-sm"></div>
          <q-btn :loading="loadingChatResponse" round color="primary" icon="send" @click="sendMessage(inputText)" />
        </div>
      </div>
    </q-card-section>
  </q-card>
